...
export function validateHand(tiles: TileDef[]): ValidationResult {
  ...
  // Check for 7 Pairs
  if (hand.length === 14) {
    const isSevenPairs = Object.values(idCounts).every(c => c === 2 || c === 4);
    if (isSevenPairs) {
      return { isValid: true, reason: 'Winning Hand: 7 Pairs', type: '7 Pairs' };
    }
  }
  ...
  if (isStandardWin) {
    return { isValid: true, reason: 'Winning Hand: 4 Sets + 1 Pair', type: 'Standard' };
  }
  ...
  if (pungsCount > 0 || sequences.length > 0) {
    ...
    if (totalSets >= 1 && totalSets < 4) {
      return { isValid: false, reason: `Looking good! Add ${4 - totalSets} more sets and a pair.` };
    }
  }

  if (pairsCount >= 2 && pairsCount < 7) {
    return { isValid: false, reason: `Nice pairs! Add ${7 - pairsCount} more pairs for 7 Pairs win.` };
  }

  if (hand.length < 14) {
    return { isValid: false, reason: `Current Hand: ${hand.length}/14 tiles. Keep going!` };
  }

  return { isValid: false, reason: 'Not a winning hand yet. Try arranging into sets and pairs.' };
}
...
function canDecompose(counts: Record<string, number>): boolean {
  const ids = Object.keys(counts).filter(id => counts[id] > 0).sort();
